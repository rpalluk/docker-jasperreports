...
jasperreports_run_deployment_scripts() {
    ...
    if am_i_root; then
    info "Executing deployment scripts: i'm root"
        ...
        chmod o-w "$BITNAMI_ROOT_DIR/tomcat/lib"   
        info "End deployment scripts: i'm root"
    else
    info "Executing deployment scripts: i'm not root"
        debug_execute "${JASPERREPORTS_CONF_DIR}/js-ant" "deploy-webapp-ce"
    info "End deployment scripts: i'm not root"
    fi
    
    info "End deployment scripts"
}
